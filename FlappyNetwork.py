...
while (True):
    if display():
        ...
        if speciesNumber == len(generation) - 1:
            ...
            # idk maybe just kill bottom 3/4 and give the top 1/4 ten children each??????
            # yea sounds good (enough) to me
            newGeneration = []
            generation = mergeSort(generation)
            newGeneration.append(generation[0][0])

            for i in range((len(generation) // 4) - 1):
                for _ in range(4):
                    newGeneration.append(generation[i][0].mutate())
            while len(newGeneration) <= generationSize:
                newGeneration.append(generation[0][0].mutate())
